...
class PageHtmlGenerator {
	...
	public function generate() {
		...
		for($i=0; $i<$pageElementNum; $i++) {
			...
			// サービス引数入力フォーム HTML
			else if($kind=='Service Argument Input Form') {
				// このサービス引数フォームのタイプを調べる
				$query_checkSaifType = "select * from page_element_saif where pePrimaryKey=".$pePrimaryKey;
				$result_checkSaifType = mysql_query($query_checkSaifType);
				oneResultCheck($result_checkSaifType, get_class($this), __FUNCTION__);
				$saifType = mysql_result($result_checkSaifType, 0, "kind");

				// サービス引数フォームの名前を出力
				$saifName = mysql_result($result_checkSaifType, 0, "saifName");
				$html .= $saifName."<br><br>";
				...
				// DATE型
				if($saifType==="date") {
					$html .= $this->getDateSaifHtml($pePrimaryKey);
				}
			}
			// ログインフォーム HTML
		...
		if(!$clientInfo->isGuest()) {
			...
			if(JAPANESE)
				...
				$html .= "Logging in as " . $clientRoleName . "<br>\n";
			...
			if(JAPANESE)
				...
				$html .= "<input type='button' value='Logout' onClick='" . $javaScriptCode . "'>\n";
		}
	...
	public function getDateSaifHtml($pePrimaryKey) {
		$html = "";

		// YEAR
		$html .=	"<select name='" . $this->getDateSaifYearRequestName($pePrimaryKey) . "'>\n";
		$currentDate = date("Y");
		for($i=1900; $i<=2100; $i++) {
			$html .= "<option value='" . $i . "'";
			if($i==$currentDate) {
		...
		// MONTH
		$html .= "<select name='" . $this->getDateSaifMonthRequestName($pePrimaryKey) . "'>\n";
		$currentMonth = date("m");
		for($i=1; $i<=12; $i++) {
			// 1～9には0をつける
			if($i<10) {
		...
		// DATE
		$html .= "<select name='" . $this->getDateSaifDateRequestName($pePrimaryKey) . "'>\n";
		$currentDate = date("d");
		for($i=1; $i<=31; $i++) {
			// 1～9には0をつける
			if($i<10) {
	...
	public function getDateSaifYearRequestName($pePrimaryKey) {
		return "saif_".$pePrimaryKey."_year";
	}
	public function getDateSaifMonthRequestName($pePrimaryKey) {
		return "saif_".$pePrimaryKey."_month";
	}
	public function getDateSaifDateRequestName($pePrimaryKey) {
		return "saif_".$pePrimaryKey."_date";
	}
}
?>
